        ...
        try:
            ...
            if "gemini" in model_name:
                try:
                    ...
                    with open(f"debugs/gemini_raw_response_{agent_type.lower()}.txt", "w") as f:
                        f.write(str(content))
                    ...
                    try:
                        ...
                        # If not valid JSON, attempt to fix
                        logger.warning(f"Invalid JSON from Gemini. Attempting to fix response.")
                        # Wrap in proper JSON if necessary
                        content = json.dumps({"response": content})
                except Exception as e:
                    logger.error(f"Error with Gemini API call: {str(e)}")
                    content = json.dumps({"error": str(e), "response": "I apologize, but I couldn't process your request properly."})
            ...
            return json.dumps({"error": str(e), "response": "I apologize, but there was an error processing your request."})
        ...
        try:
            ...
            if "next_agent" in zene_response and zene_response["next_agent"].lower() == "commet":
                ...
                try:
                    ...
                    return {"error": f"Failed to parse Commet response: {str(e)}", "response": commet_response_str}
            ...
            if "vector_database_retrieval_queries" in zene_response:
                ...
                for query in quiries[:1]:
                    ...
                    try:
                        ...
                        rag_responses.append({"error": str(e), "raw_response": res})
            ...
            if "next_agent" in zene_response and zene_response["next_agent"].lower() == "milo":
                ...
                try:
                    ...
                    return {"error": f"Failed to parse Milo response: {str(e)}", "response": milo_response_str}
            ...
            if "next_agent" in zene_response and zene_response["next_agent"].lower() == "thalia":
                ...
                try:
                    ...
                    return {
                        "error": f"Failed to parse Thalia response: {str(e)}", 
                        "response": "I apologize, but I couldn't process your request properly.",
                        "raw_response": thalia_response_str[:500]  # Include first 500 chars of raw response
                    }
            ...
            return {"error": f"Failed to parse response: {str(e)}", "response": "I apologize, but I couldn't process your request properly."}
