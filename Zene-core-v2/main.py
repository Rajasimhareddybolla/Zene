...
# Local imports
from prompts import Zene, summary, user_knowledge, Commet, Finn, Milo, Thalia , Mara , Inka
...
class Mioo:
    ...
    def __init__(self, user_id: str, db_url: Optional[str] = None):
        ...
        self.agent_types = ["zene", "commet", "finn", "milo", "thalia" , "mara" , "inka"]
    ...
    def _route_to_agent(self, 
        ...
        if rag_responses and type(rag_responses) == str:
            augmented_message = f"{message}\nAdditional context: {rag_responses}"
        elif rag_responses:
            augmented_message = f"{message}\nAdditional context: {json.dumps(rag_responses)}"
        
        # Set model based on agent
        model_name = "o3-mini" if next_agent == "thalia" else DEFAULT_MODEL
    ...
    def __call__(self, message: str) -> Tuple[Union[str, Dict[str, Any]], List[Dict[str, Any]]]:
        ...
        try:
            ...
            rag_responses = []
            if "vector_database_retrieval_queries" in zene_response and "next_agent" is not "commet":
            ...
            if "next_agent" in zene_response:
                ...
                if zene_response["next_agent"].lower() != "commet"  and success:
                    ...
                    async def get_agent_responses():
                        ...
                        mara_result, inka_result = await asyncio.gather(mara_task, inka_task)
                        return mara_result, inka_result
                    
                    # Run the async function and get results
                    mara_response, inka_response = asyncio.run(get_agent_responses())
                    
                    # Combine responses into a single JSON response
                    response_mara = {
                        "mara_response": mara_response[0] if mara_response[1] else {"error": "Mara agent failed"},
                        "inka_response": inka_response[0] if inka_response[1] else {"error": "Inka agent failed"},
                        "success": mara_response[1] and inka_response[1]
                    }
                    if success:
                        return response_mara, self.conversation_flow
